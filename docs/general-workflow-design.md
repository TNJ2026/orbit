...
当前阶段三实现已提供上述 Decision：节点的 `input_schema` 先校验映射后的输入，
`rules` 按声明顺序求值，首个命中规则选择其 `port`，全部未命中时选择
`default_port`；Decision 由引擎同步执行，不创建 Agent 消息或 runner job。实现固定
声明 `expression_language: {"name":"jsonlogic","version":"1","profile":"orbit-safe-v1"}`，
保存和读取配置时拒绝未知操作符或不同语言/profile。安全子集当前开放比较、
`and` / `or` / `!` / `!!`、`var`、`length`、`in` 和 `missing`。
...
```json
{
  ...
  "items": "$.nodes.plan.latest.output.items",
  "item_key": "$.id",
...
当前已落地并开放第一版 foreach runtime：
`workflow_item_groups` 为每次 activation 保存父 scope、并发上限与整体状态，
`workflow_item_scopes` 保存稳定 key、原始 item、兄弟依赖、独立输出及生命周期状态。
创建 group 时会拒绝重复 key、未知依赖、自依赖和依赖环；无依赖 item 按
`max_concurrency` 进入 `ready`，item 完成后在同一数据库事务中释放新的就绪兄弟。
每个 ready scope 创建带 `item_scope_id` 的 runner job；同一 foreach 节点的 Agent 与
Prompt 就是当前第一版 item body，runner 从 `FOREACH_ITEM_SCOPE` 读取 item，上下文映射
使用 `$.scope.item` 与 `$.scope.item_meta`。成功输出按 item 顺序聚合后父节点只推进一次；
失败会阻塞 group 并取消尚未开始的兄弟。scheduler 会恢复 ready scope、失败 job 和
completed-but-unadvanced 崩溃窗口。配置 schema 与 UI 已开放 items/key/dependency、并发
和单项 output schema。任意多节点 Subflow 作为 body 仍属于后续 Subflow 能力。
...
当前 task runtime 已开放显式 `join` 节点的第一批能力：`join_policy` 支持
`all_activated`（`all` 为配置别名）和 `any`；`aggregation` 支持 `list`、
`object_by_source` 和 `first`。Join 由引擎执行，不占用 Agent，其结构化输出固定为：

```json
{
  "inputs": [{"source": "a", "port": "success", "output": {}, "summary": "", "artifacts": []}],
  "value": "由 aggregation 决定的聚合值"
}
```

`inputs` 始终保留完整来源与结果，`value` 提供便于消费的聚合视图；下游映射可使用
`$.join.inputs[0].output` 或 `$.join.inputs[*].output`。task runtime 已为直接汇入同一
Join 的 fork/decision 分支持久化 correlation group：每个 group 记录单调递增的
activation、创建时 transition cursor、各前驱分支状态及 consumed 状态。Join 只聚合
cursor 之后的本轮到达结果，因此循环重新进入 fork 后不会复用上一轮 arrival 或输出；
进程重启后也可从数据库继续判定。`any` 每个 activation 只触发一次，迟到分支仍可
完成但不会重复激活该次 Join。`quorum`、`count`、`all_successful`、remaining cancel，
以及 foreach/subflow scope 的跨作用域 lineage 仍随完整 token runtime 实现。task-local
嵌套 fork 已通过 `parent_correlation_id` 把内层 Join activation 关联到可达的外层 Join
activation；同级内层 Join 分别关联外层组，不互相形成错误的父子关系。
...
同一来源端口可以同时具有条件边和无条件 fallback 边。条件使用同一
`orbit-safe-v1` JSONLogic，在来源节点的 scoped result context 上求值；命中的最低
`priority` 组被选择，同优先级的多条命中边形成并行 fork。没有条件命中时才选择该
端口的全部无条件边。未选择的直接出边记录 `not_selected` 审计事件；Decision 还会
关闭其他未选端口。当前 task runtime 会沿未选分支的前向子图传播闭合事件，到达第一
个多入边汇合边界后停止，因此普通无循环的互斥分支已经不会让现有隐式 Join 永久
等待。当前已落地 task-local 的持久化 correlation group：fork/decision 到显式 Join
边界的分支选择、到达、未选择、取消/阻塞状态持久化，并按 activation 隔离循环旧事件。
边界发现会继续越过内层 Join 寻找外层 Join，并持久化父子 correlation lineage，因此
task-local 嵌套汇合不会提前消费外层 activation。foreach 已有独立持久化 item scope，
但 correlation group 尚不能跨 foreach scope 或 Subflow run 传播 lineage；通用 token
传递也未实现，因此跨作用域汇合仍不能宣称具备完整隔离。
...
当前兼容实现使用单行 JSON envelope：

```text
WORKFLOW_RESULT: {"port":"approved","output":{"artifact":"article.md"},"summary":"Review passed","artifacts":[{"type":"file","uri":"article.md"}]}
```

存在 `output_schema` 时该 envelope 为必需；JSON 中的 `port` 优先于 `WORKFLOW_PORT`。没有 `output_schema` 的旧节点继续按 `RESULT_SUMMARY`、`ARTIFACTS` 和原始文本规范化，`output` 为空对象。
...
当前 task runtime 已实现其中的 task-local 子集：`$.run.inputs`、`$.run.variables`、
`$.scope.item`、`$.scope.iteration`、`$.nodes.<id>.latest` 和
`$.nodes.<id>.runs[n]`，并保留 `$.output` / `$.summary` / `$.artifacts` /
`$.port` 作为当前节点结果的简写。引用不会越过当前 task（即当前 workflow
run）。普通 task 节点的 `scope.item` 仍兼容映射为当前 task；foreach item runner 则由
持久化 `workflow_item_scopes` 提供真正的 `scope.item` / `scope.item_meta`。
`scope.iteration` 是当前节点从 0 开始的执行序号。显式 Join 已支持
`$.join.inputs[*]`；跨 Subflow/run 的 scope 寻址仍待完整 token runtime。
...
当前过渡实现已先增加 `workflow_correlations` 与
`workflow_correlation_branches`，把 Join activation、transition cursor 和分支闭合状态
从 transition 推断结果提升为持久化事实。这是 token runtime 的兼容子集，不等同于完整
的 `workflow_runs` / `node_runs` / `workflow_tokens`：它仍以现有 task 为 run scope，且只
在静态 task-local 图中可识别的 fork/decision → 显式 Join 边界创建 group。嵌套组通过
`parent_correlation_id` 关联，但尚不能跨 foreach item 或 Subflow run 传播 lineage。

foreach 过渡层另有 `workflow_item_groups` / `workflow_item_scopes`：它已经持久化
activation、父 scope、并发槽、item 值、依赖 DAG、状态和独立输出，并能在 item 完成后
事务化释放新的 ready scope。`run_jobs.item_scope_id` 建立逐 item 执行归属，scheduler
负责结果回写、失败传播、后续 item 释放、最终聚合和恢复；该能力已通过配置 schema 与
UI 对作者开放。它仍复用现有 task/task_run 投影，尚未升级为独立 `node_runs`。
...
For-each（单节点 item body）和 Join 首批策略已落地；任意多节点 Subflow body 与完整
token runtime 在后续版本实现。

### 17.1 当前实现状态（2026-07-15）

阶段一和阶段二的核心能力已经落地：

- 默认模板显式声明 `type`、`handler`、`executor`、`environment`、`exclusive`、`contract`、`enabled`、`removable` 和 `skippable`；新 schema 节点不再根据 ID 获得执行特权，旧 ID 映射只存在于 v1 兼容层；
- Runner 使用 `workspace_access` 和 `inject_workflow_snapshot` 等能力字段生成执行上下文，不再通过 `review`、`test`、`intake` 名称推断行为；
- Handler 注册表向 API 暴露可创作 schema，UI 根据服务端返回的节点类型、Handler、Agent 数量和 command 能力生成编辑选项；
- `action`、独立 `approval`、`end`，自定义 outcome port、保留错误端口、未路由策略和边级循环限制已具备运行时语义；
- `skippable` 已由运行时强制执行，`removable` 由编辑器强制执行；旧工作流继续兼容读取和执行。
- 软件开发、内容发布、数据处理三个模板已内置；模板只生成通用节点配置，可通过 Workflow API 和 UI 显式应用，且不会预设 Agent。

近期交付范围已经完成。下一阶段进入结构化输入输出、字段映射和 Decision 所需的数据基础。开发专用的“重新实现”、Git、自动测试检测和默认 prompt contract 仍按阶段五迁出核心。

阶段三的数据基础已实现：节点可声明 `input_schema` / `output_schema`；Runner 支持 `WORKFLOW_RESULT` JSON envelope；每次节点完成会追加持久化的结构化 result；边可声明 `mapping`，并在目标调度前完成字段映射和 input schema 校验。节点还可通过 `retry.normalization`（0–5）配置规范化重试；失败 attempt 使用 `reassigned` 审计事件在同一 activation 上重新派发，不增加业务边循环计数，耗尽后才进入 `blocked`。映射已支持 task-local scoped 引用和节点执行历史。条件边和引擎内执行的 Decision 节点已支持固定 `orbit-safe-v1` JSONLogic，并在 UI 中开放创作。task-local 条件分支会把 `not_selected` 传播到第一个汇合边界，现有隐式 Join 能正确闭合未选分支。显式 Join 已支持 `all_activated` / `any`、三种聚合视图和 `$.join.inputs[*]`。fork/decision → 显式 Join 的 correlation group、分支状态、transition cursor、activation 和 consumed 状态已持久化，循环再次进入时不会复用旧轮次 arrival 或聚合输出；task-local 嵌套 Join 还会持久化父子 correlation lineage，内层完成不会提前消费外层 activation。foreach 已连接持久化 item group/scope、依赖 DAG、并发就绪释放、item-local runner job、失败传播、顺序聚合和重启恢复，并已开放配置 schema 与 UI。待完整 token runtime 补齐的是多节点 Subflow body、其余 Join policy、跨 Subflow/run lineage，以及独立的 workflow run / node run / token 生命周期。
